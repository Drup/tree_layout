...
(** Implementation of
    ...
    Based on the implementation in {!Tree_layered}.
*)
module Make (G : TREE) = struct
  ...
  type state = {
    ancestor : G.V.t H.t ;
    thread_left : G.V.t H.t ;
    thread_right : G.V.t H.t ;
    modtbl : float H.t ;
    mser : float H.t ;
    msel : float H.t ;
    prelim : float H.t ;
  ...
  module Iyl = struct
    type cell = { lowY : float ; index : int }
    type t = cell list ref

    let rec drop minY = function
      | { lowY } :: t when minY >= lowY -> drop minY t
      | l -> l
    let update lowY index iyl =
      iyl := { lowY ; index } :: drop lowY !iyl
    let init s v = ref [ { lowY = s.bottom v ; index = 0 } ]
    let drop1 l = match !l with
      | [] -> assert false
      | _ :: t -> l := t
    let lowY l = match !l with
      | [] -> assert false
      | { lowY } :: _ -> lowY
  end
  ...
  let next_right s v =
    ...
    | Some w -> Some w
    | None -> find s.thread_right v

  let next_left s v =
    match G.leftmost_child s.g v with
    | Some w -> Some w
    | None -> find s.thread_left v
  ...
  let add_child_spacing ~s parent =
    let d = ref 0. and modsumdelta = ref 0. in
    G.children s.g parent (fun v ->
      d := !d +. H.find s.shift v ;
      modsumdelta := !modsumdelta +. !d +. H.find s.change v ;
      incr s.modtbl v !modsumdelta ;
    )

  let move_subtree ~s ~parent wm wp dist =
    let subtrees = float @@ number ~s ~parent wp - number ~s ~parent wm in
    incr s.change wp @@ dist -. dist /. subtrees ;
    incr s.shift  wp @@ ~-. dist /. subtrees ;
    incr s.change wm @@ dist /. subtrees ;

    incr s.msel wp @@ dist ;
    incr s.mser wp @@ dist ;
    incr s.modtbl wp @@ dist ;
    ()

  let position_root ~s v =
    match G.leftmost_child v, G.rightmost_child v with
    | Some lv, Some rv ->
      H.add s.prelim v @@
      H.add
  ...
  let apportion ~s ~ih ~parent ~sibling ~defaultAncestor v =
    match sibling with
    ...
    | Some w ->
      ...
      let rec aux () = match next_right s !vim, next_left s !vip with
        | Some vim_, Some vip_ ->
          ...
          if s.bottom vim_ > Iyl.lowY ih then Iyl.drop1 ih else () ; (* NEW *)
          H.add s.ancestor !vop v ;
          let shift =
            (get_prelim s vim_ +. !sim)
            -. (get_prelim s !vip +. !sip)
            +. s.distance vim_ !vip
          in
          if shift > 0. then begin
            move_subtree ~s ~parent
              (ancestor ~s ~defaultAncestor ~parent vim_) v shift ;
            sip := !sip +. shift ;
            sop := !sop +. shift ;
          end ;
          let vim_y = s.bottom vim
  ...
  let rec first_walk ~s ~sibling v =
    ...
    | Some vl -> begin
        ...
        let ih = Iyl.init s vl in (* NEW *)
        Seq.fold_sibling
          (fun defaultAncestor sibling w ->
             first_walk ~s ~sibling w ;
             let minY = s.bottom w in (* NEW *)
             let newAncestor =
               apportion ~s ~parent:v
                 ~ih
                 ~defaultAncestor
                 ~sibling w in
             Iyl.update minY 0 ih ; (* NEW *)
             newAncestor
          )
          vl (G.children s.g v) ;
        execute_shifts s v ;
  ...
  let layout ~distance ~bottom g r =
    ...
    let iyl = [ { index = 0 ; lowY = bottom @@ r } ] in
    let s =
